                ...
                <tr class="text-gray-700 dark:text-gray-400">
                  <td class="px-4 py-3">
                    <div class="flex items-center text-sm">
                      <div>
                        <p class="font-source-code-pro font-semibold bg-secondary focus:outline-none focus:border-none focus:ring-0 w-full h-auto"><?php print $bron["content"] ?></p>
                      </div>
                    </div>
                  </td>
                  <td class="px-4 py-3 text-sm">
                    <p class="font-source-code-pro font-semibold bg-secondary focus:outline-none focus:border-none focus:ring-0 w-full h-auto"><?php print $bron["url"] ?></p>
                  </td>
                  <td class="px-4 py-3">
                    <div class="flex items-center space-x-4 text-sm">
                      <form action="" method="POST">
                        <button type="submit" name="remove" value="<?php print $bron["id"] ?>" class="flex items-center text-white bg-black px-4 py-2 justify-between text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray" aria-label="Delete">
                          Verwijderen
                        </button>
                      </form>
                    </div>
        ...
        <div class="mt-4 mb-6">
          ...
          <form method="post" action="">
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-1">
              ...
              <div class="">
                <label for="type" class="block text-sm font-medium leading-5 text-black">
                  Bron type
                </label>
                <div class="mt-1 rounded-md shadow-sm">
                  <select name="type" id="type" onchange="dropdown('type', 'url')" class="bg-white form-input block w-full transition duration-150 drop-shadow-xl ease-in-out sm:text-sm sm:leading-5 p-2 rounded">
    ...
    <script>
      function dropdown(id, url) {
        ...
        if (x.value == "url") {
          ...
          y.value = "";
          // Set the url input to enabled
          y.disabled = false;
        } else {
          // Set the value to string 
          y.value = "Niet van toepassing";
          // Set the url input to disabled
          y.disabled = true;
        }
      }
