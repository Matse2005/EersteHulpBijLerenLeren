...
if (!subcategory($db, $_GET['id'])) exit;
else $subcategory = subcategory($db, $_GET['id']);
if (isset($_POST["submit"])) subcategory_edit($db, $_GET['id'], $_POST["title"], $_POST["category"], $_POST["content"]);
?>
  ...
  <form action="" method="post" class="w-full px-5 sm:px-5 md:px-5 h-full overflow-y-auto">
    <div class="px-1 md:px-6 mx-auto ">
      <div class="grid grid-cols-4">
        <input name="title" placeholder="Categorie titel..." value="<?php print $subcategory["title"] ?>" class="my-6 col-span-3 text-xl font-semibold bg-secondary border-none focus:outline-none focus:border-none focus:ring-0" />
      ...
      <div class="w-full overflow-hidden rounded-lg shadow-xs">
        ...
        <div class="w-full bg-secondary resize-none border-none focus:outline-none focus:border-none focus:ring-0">
          <select name="category" id="category" class="w-full bg-secondary resize-none border-none focus:outline-none focus:border-none focus:ring-0">
            <option value="<?php print $subcategory["category"] ?>" selected><?php print category_get_by_id($db, $subcategory["category"])["title"] ?></option>
            <?php
            foreach (categories($db) as $categories) {
              if ($categories["id"] == $subcategory["category"]) continue;
            ?>
        ...
        <div class="editor mt-5">
          <textarea name="content" placeholder="Begin met de uitleg over deze subcategorie..." id="editor" cols="30" rows="10"><?php print $subcategory["description"] ?></textarea>
        </div id="editor">
      </div>
    </div>
  </form>
